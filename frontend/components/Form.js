...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const sizeText = formValues.size === 'S' 
    ? 'small' : formValues.size === 'M' ? 'medium' : 'large';
    const toppingsText = numberOfToppings === 0 
    ? 'no toppings' 
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {submitSuccess === false && <div className='failure'>Something went wrong</div>}
      ...
      <input 
      type="submit" 
      disabled={Object.keys(formErrors).length > 0 || !formValues.fullname || !formValues.size || isSubmitting} />
    </form>
  )
}
